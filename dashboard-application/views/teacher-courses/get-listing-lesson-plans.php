...
<div class="box -padding-20">
	<a href="<?php echo CommonHelper::generateUrl('TeacherLessonsPlan'); ?>" class="left" >Add New Lesson Plan</a>
	<input type="button" value="Save" onclick="assignCoursesToLessonPlan('<?php echo $courseId; ?>');" />
	<div class="table-scroll">
	 <table class="table">
		...
		<?php $i=1; foreach($lessonsPlanData as $lessonPlanData){ ?>
		<tr>
			<td>
				<span class="td__data"><?php echo $i; ?></span>
				<input <?php if(in_array($lessonPlanData['tlpn_id'],$rowsCorseRelToPlan)){ echo 'checked="checked"';} ?> type="checkbox" name="selecte_plans[]" value="<?php echo $lessonPlanData['tlpn_id']; ?>" />
			</td>
			<td>
			...
			<!--td width="35%">
				<span class="td__data">
					<?php echo substr($lessonPlanData['tlpn_description'],0,150); ?>
				</span>
			</td>
			<td width="35%">
				<span class="td__data">
				<ul class="tags">
				<?php $tags =  explode(",",$lessonPlanData['tlpn_tags']);
						foreach($tags as $tag){
							echo '<li><a class="tage"><span>'.$tag.'&nbsp;&nbsp;</a></li>';
						}
					?>
					</ul>
				</span>
			...
			<!--td>
				<a href="javascript:void(0);" onclick="assignCoursesToLessonPlan('<?php echo $lessonId; ?>','<?php echo $lessonPlanData['tlpn_id']; ?>')"><?php echo Label::getLabel('LBL_Assign'); ?></a>
			</td-->
