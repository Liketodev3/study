    ...
    <a href="javascript:void(0)" class="nav__dropdown-trigger nav__dropdown-trigger-js">
        <div class="avtar avtar--xsmall -display-inline" data-text="<?php echo CommonHelper::getFirstChar(UserAuthentication::getLoggedUserAttribute('user_first_name')); ?>">
			...
			if( true == User::isProfilePicUploaded() ){
				echo '<img src="'.CommonHelper::generateUrl('Image','user', array( UserAuthentication::getLoggedUserId() ), CONF_WEBROOT_FRONT_URL).'?'.time().'" alt="" />';
			}
			?>
        </div>
		...
		<div class="nav__dropdown-target nav__dropdown-target-js -skin">
			...
			<nav class="nav nav--vertical -hide-responsive">
				<ul>
					...
					<li><a href="<?php echo CommonHelper::generateUrl('GuestUser','logout', [], CONF_WEBROOT_FRONT_URL);?>"><?php echo Label::getLabel('LBL_Logout'); ?></a></li>
				</ul>
			</nav>
...
<li class="-hide-mobile">
	<a href="javascript:void(0)" onClick="signUpFormPopUp();"><?php echo Label::getLabel('LBL_Sign_Up'); ?><img src="<?php echo CONF_WEBROOT_FRONT_URL; ?>images/user.svg" alt="<?php echo Label::getLabel('LBL_Sign_Up'); ?>" class="-hide-desktop -show-mobile"></a>
</li>

<li class="user-click">
	<a href="javascript:void(0)" onClick="logInFormPopUp();" class="btn btn--primary"><?php echo Label::getLabel('LBL_Login'); ?>
		<span class="svg-icon user-icon">
