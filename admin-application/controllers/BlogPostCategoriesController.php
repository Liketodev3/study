...
class BlogPostCategoriesController extends AdminBaseController
{
    ...
    public function __construct($action)
    {
        ...
        if (!FatUtility::isAjaxCall() && in_array($action, $ajaxCallArray)) {
            die(Label::getLabel('MSG_Invalid_Action', $this->adminLangId));
        }
    ...
    public function getBreadcrumbNodes($action)
    {
        ...
        switch ($action) {
            case 'index':
                $nodes[] = array('title'=>Label::getLabel('LBL_Root_categories', $this->adminLangId), 'href'=>CommonHelper::generateUrl('BlogPostCategories'));
    ...
    private function getForm($bpcategory_id = 0)
    {
        ...
        $frm->addSelectBox(Label::getLabel('LBL_Category_Parent', $this->adminLangId), 'bpcategory_parent', array(0=>'Root Category') + $categories, '', array('class'=>'small'), '');
        $activeInactiveArr = applicationConstants::getActiveInactiveArr($this->adminLangId);
        $frm->addSelectBox(Label::getLabel('LBL_Category_Status', $this->adminLangId), 'bpcategory_active', $activeInactiveArr, '', array('class'=>'small'), '');
        $frm->addCheckBox(Label::getLabel('LBL_Featured', $this->adminLangId), 'bpcategory_featured', 1, array(), false, 0);
