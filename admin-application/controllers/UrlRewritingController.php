...
class UrlRewritingController extends AdminBaseController
{
    ...
    public function form($urlrewrite_id = 0)
    {
        ...
        $frm = $this->getForm();
        $frm->fill(['urlrewrite_id' => $urlrewrite_id]);
        if (0 < $urlrewrite_id) {
    ...
    public function deleteRecord()
    {
        ...
        $res = UrlRewrite::getAttributesById($urlrewrite_id, ['urlrewrite_id']);
        if ($res == false) {
    ...
    private function getForm($urlrewrite_id = 0)
    {
        ...
        $fld = $frm->addRequiredField(Label::getLabel('LBL_Custom_URL', $this->adminLangId), 'urlrewrite_custom');
