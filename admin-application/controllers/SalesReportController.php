...
class SalesReportController extends AdminBaseController
{
    ...
    public function export()
    {
        ...
        $srch->addMultipleFields(array('DATE(order_date_added) as order_date','count(op_id) as totOrders','SUM(op_qty) as totQtys','SUM(op_refund_qty) as totRefundedQtys','SUM(op_qty - op_refund_qty) as netSoldQty','sum((op_commission_charged - op_refund_commission)) as totalSalesEarnings','sum(op_refund_amount) as totalRefundedAmount','sum(order_net_amount) as orderNetAmount','(SUM(op_qty*optax.opcharge_amount)) as taxTotal','(SUM(op_qty*opship.opcharge_amount)) as shippingTotal'));
        ...
        while ($row = $db->fetch($rs)) {
            ...
            $arr = array($row['totQtys'],$row['totRefundedQtys'],$row['orderNetAmount'],$row['taxTotal'],$row['shippingTotal'],$row['totalRefundedAmount'],$row['totalSalesEarnings']);
            $arr = array_merge($arr1, $arr);
            array_push($sheetData, $arr);
            $count++;
        }
