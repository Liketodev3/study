...
class SalesReportController extends AdminBaseController
{
    ...
    public function export()
    {
        ...
        $srch->addMultipleFields([
            'DATE(order_date_added) as order_date', 'count(op_id) as totOrders',
            'SUM(op_qty) as totQtys', 'SUM(op_refund_qty) as totRefundedQtys',
            'SUM(op_qty - op_refund_qty) as netSoldQty',
            'sum((op_commission_charged - op_refund_commission)) as totalSalesEarnings',
            'sum(op_refund_amount) as totalRefundedAmount', 'sum(order_net_amount) as orderNetAmount',
            '(SUM(op_qty*optax.opcharge_amount)) as taxTotal', '(SUM(op_qty*opship.opcharge_amount)) as shippingTotal'
        ]);
        $srch->addOrder('order_date', 'desc');
        ...
        $rs = $srch->getResultSet();
        $sheetData = [];
        $arr1 = [Label::getLabel('LBL_Sr_No', $this->adminLangId), Label::getLabel('LBL_Date', $this->adminLangId), Label::getLabel('LBL_No._Of_Orders', $this->adminLangId)];
        $arr2 = [Label::getLabel('LBL_Sr_No', $this->adminLangId), Label::getLabel('LBL_Invoice_Number', $this->adminLangId)];
        $arr = [Label::getLabel('LBL_No._Of_Qty', $this->adminLangId), Label::getLabel('LBL_Refund_Qty', $this->adminLangId), Label::getLabel('LBL_Order_Net_Amount', $this->adminLangId), Label::getLabel('LBL_Tex_Charges', $this->adminLangId), Label::getLabel('LBL_Shipping_Charges', $this->adminLangId), Label::getLabel('LBL_Refunded_Amount', $this->adminLangId), Label::getLabel('LBL_Sales_Earnings', $this->adminLangId)];
        if (empty($orderDate)) {
        ...
        while ($row = $db->fetch($rs)) {
            if (empty($orderDate)) {
                $arr1 = [$count, FatDate::format($row['order_date']), $row['totOrders']];
            } else {
                $arr1 = [$count, $row['op_invoice_number']];
            }
            $arr = [$row['totQtys'], $row['totRefundedQtys'], $row['orderNetAmount'], $row['taxTotal'], $row['shippingTotal'], $row['totalRefundedAmount'], $row['totalSalesEarnings']];
            $arr = array_merge($arr1, $arr);
            array_push($sheetData, $arr);
            $count++;
        }
