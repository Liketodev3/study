<?php class ContentPagesController extends AdminBaseController
{
    ...
    public function form($cpage_id = 0)
    {
        ...
        if (0 < $cpage_id) {
            $data = ContentPage::getAttributesById($cpage_id, array(
                'cpage_id',
                'cpage_identifier',
                'cpage_layout'
            ));
    ...
    public function deleteRecord()
    {
        ...
        $obj->assignValues(array(
            ContentPage::tblFld('deleted') => 1
        ));
        if (!$obj->save()) {
    ...
    public function autoComplete()
    {
        ...
        $srch->addMultipleFields(array(
            'cpage_id',
            'IFNULL(cpage_title,cpage_identifier) as cpage_name'
        ));
        $rs       = $srch->getResultSet();
        $db       = FatApp::getDb();
    ...
    private function getForm($cpage_id = 0)
    {
        ...
        $frm->addSelectBox(Label::getLabel('LBL_Layout_Type', $this->adminLangId), 'cpage_layout', $this->getAvailableLayouts(), '', array(
            'id' => 'cpage_layout'
    ...
    private function getLangForm($cpage_id = 0, $lang_id = 0, $cpage_layout = 0)
    {
        ...
        if ($cpage_layout == ContentPage::CONTENT_PAGE_LAYOUT1_TYPE) {
            $bannerTypeArr = applicationConstants::bannerTypeArr();
            $fld           = $frm->addButton(Label::getLabel('LBL_Backgroud_Image', $this->adminLangId), 'cpage_bg_image', Label::getLabel('LBL_Upload_Image', $this->adminLangId), array(
