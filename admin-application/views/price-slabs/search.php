...
foreach ($records as $sn=>$row){
	...
	foreach ($arr_flds as $key=>$val){
		...
		switch ($key){
			...
			break;
			case 'title':
                $title = Label::getLabel('{min}_to_{max}'); //5 to 9 hrs
			...
			case 'action':
				...
				if($canEdit){
					...
					'title'=>Label::getLabel('LBL_Edit',$adminLangId),"onclick"=>"priceSlabForm(".$row['prislab_id'].");"),Label::getLabel('LBL_Edit',$adminLangId), 
					true);
				}
			break;
			default:
