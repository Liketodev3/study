		...
		switch ($key){
			...
			case 'action':
				...
				if($canEdit){
					...
					'title'=>Label::getLabel('LBL_Edit',$adminLangId),"onclick"=>"editLessonPackageFormNew(".$row['lpackage_id'].")"),Label::getLabel('LBL_Edit',$adminLangId), 
					true);
					
					if( $row['lpackage_is_free_trial'] == 0 ){
						$innerLiDelete = $innerUl->appendElement("li");
						$innerLiDelete->appendElement('a', array('href'=>'javascript:void(0)', 'class'=>'button small green', 
						'title'=>Label::getLabel('LBL_Delete',$adminLangId),"onclick"=>"deleteRecord(".$row['lpackage_id'].")"),Label::getLabel('LBL_Delete',$adminLangId), 
						true);
					}
				}
			break;
