...
switch ($frmType){
	...
	case Configurations::FORM_MEDIA:
		...
		$faviconFld = $frm->getField('favicon');
		$mobilelogoFld = $frm->getField('mobile_logo');
		...
		$mobilelogoFld->htmlAfterField = '<span class = "uploadimage--info" >&nbsp;</span>';
		...
		if( AttachedFile::getAttachment( AttachedFile::FILETYPE_WATERMARK_IMAGE, 0, 0, $lang_id ) ){
			$watermarkFld->htmlAfterField .= '<div class="uploaded--image"><img src="'.FatUtility::generateFullUrl('Image','watermark',array($lang_id), CONF_WEBROOT_FRONT_URL).'?'.time().'"> <a  class="remove--img" href="javascript:void(0);" onclick="removeWatermark('.$lang_id.')" ><i class="ion-close-round"></i></a></div><br>';
		}
		...
		if( AttachedFile::getAttachment( AttachedFile::FILETYPE_MOBILE_LOGO, 0, 0, $lang_id ) ){
			$mobilelogoFld->htmlAfterField .= '<div class="uploaded--image"><img src="'.FatUtility::generateFullUrl('Image','socialFeed',array($lang_id , 'THUMB'), CONF_WEBROOT_FRONT_URL).'?'.time().'"><a  class="remove--img" href="javascript:void(0);" onclick="removeMobileLogo('.$lang_id.')" ><i class="ion-close-round"></i></a></div><br>';
		}
	break;
