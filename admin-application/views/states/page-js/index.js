...
(function() {
	...
	activeStatus = function(obj){
		...
		data='stateId='+stateId+'&status='+active;
		...
		var ans =$.parseJSON(res);
			if( ans.status == 1 ){
				...
				$(obj).addClass("active");
				$(".status_"+stateId).attr('onclick','inactiveStatus(this)');
			}
			else{
	...
	inactiveStatus = function(obj){
		...
		data='stateId='+stateId+'&status='+inActive;
		fcom.ajax(fcom.makeUrl('States','changeStatus'),data,function(res){
		var ans =$.parseJSON(res);
			if( ans.status == 1 ){
				fcom.displaySuccessMessage(ans.msg);				
				...
				$(".status_"+stateId).attr('onclick','activeStatus(this)');
			}
			else{
				fcom.displayErrorMessage(ans.msg);				
			}
