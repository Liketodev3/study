...
(function() {
	...
	mediaForm = function(blocationId,bannerId){
		fcom.displayProcessing();
			fcom.ajax(fcom.makeUrl('Banners', 'mediaForm', [blocationId,bannerId]), '', function(t) {
				images(blocationId,bannerId,0,0);
			fcom.updateFaceboxContent(t);
			});
	};
	...
	images = function(blocationId,bannerId=0,langId=0,screen=0){
		...
		fcom.ajax(fcom.makeUrl('Banners', 'images', [blocationId,bannerId,langId,screen,true]), '', function(t) {
			$('#image-listing-second').html(t);
			fcom.resetFaceboxHeight();
		});        
	};
	
	removeBanner = function(blocationId,bannerId,langId,screen,secondary=0){
		if( !confirm(langLbl.confirmDeleteImage) ){ return; }
		fcom.updateWithAjax(fcom.makeUrl('Banners', 'removeBanner',[bannerId,langId,screen,secondary]), '', function(t) {
			images(blocationId,bannerId,langId,screen);
			reloadList();
		});
	};
	...
	activeStatus = function( obj ){
		...
		data = 'bannerId='+bannerId+"&status="+active;
		fcom.ajax(fcom.makeUrl('Banners','changeStatus'),data,function(res){
			...
			if(ans.status == 1){
				...
				$(obj).addClass("active");
				$(".status_"+bannerId).attr('onclick','inactiveStatus(this)');
			}else{
	...
	inactiveStatus = function( obj ){
		...
		data = 'bannerId='+bannerId+"&status="+inActive;
		fcom.ajax(fcom.makeUrl('Banners','changeStatus'),data,function(res){
			var ans =$.parseJSON(res);
			if(ans.status == 1){
				$.mbsmessage(ans.msg,true,'alert--success');
				...
				$(".status_"+bannerId).attr('onclick','activeStatus(this)');
			}else{
				$.mbsmessage(ans.msg,true,'alert--danger');
			}
		});
...
$(document).on('click','.bannerFile-Js',function(){
	...
	frm = frm.concat('<input type="hidden" name="banner_image" value="'+banner_image+'"/>'); 
	$('body').prepend(frm);
