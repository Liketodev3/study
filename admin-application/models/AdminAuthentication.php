...
class AdminAuthentication extends FatModel
{
    ...
    {
        $db = FatApp::getDb();
        if (!$db->deleteRecords('tbl_admin_password_reset_requests', array(
            'smt' => 'aprr_expiry < ?',
            'vals' => array(
                date('Y-m-d H:i:s')
            )
    ...
    public function addPasswordResetRequest($data = array())
    {
        ...
        $db = FatApp::getDb();
        if ($db->insertFromArray('tbl_admin_password_reset_requests', array(
            'aprr_admin_id' => intval($data['admin_id']),
            'aprr_token' => $data['token'],
            'aprr_expiry' => date('Y-m-d H:i:s', strtotime("+1 DAY"))
        ))) {
            $db->deleteRecords('tbl_admin_auth_token', array(
    ...
    public function changeAdminPwd($aId, $pwd)
    {
        ...
        if ($db->updateFromArray('tbl_admin', $data, array(
            'smt' => 'admin_id=?',
            'vals' => array(
                $aId
            )
        ))) {
            $db->deleteRecords('tbl_admin_password_reset_requests', array(
                'smt' => 'aprr_admin_id=?',
                'vals' => array(
                    $aId
                )
    ...
    public static function clearLoggedAdminLoginCookie()
    {
        ...
        if (strlen($_COOKIE[static::ADMIN_REMEMBER_ME_COOKIE_NAME])) {
            $db->deleteRecords('tbl_admin_auth_token', array(
                'smt' => 'admauth_token = ?',
                'vals' => array(
                    $_COOKIE[static::ADMIN_REMEMBER_ME_COOKIE_NAME]
                )
            ));
        }
