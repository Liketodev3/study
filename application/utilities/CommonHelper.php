...
class CommonHelper extends FatUtility{
	...
	public static function initCommonVariables( $isAdmin = false ){
		...
		$currencyData = Currency::getAttributesById(self::$_currency_id,
	...
	static function generateFullUrl($controller = '', $action = '', $queryData = array(), $use_root_url = '', $url_rewriting = null, $encodeUrl = false){
	...
	public static function getCurrencySymbol($showDefaultSiteCurrenySymbol = false){
		if($showDefaultSiteCurrenySymbol){
			...
			$currencyData = Currency::getAttributesById($currency_id,array('currency_symbol_left','currency_symbol_right')
			);
	...
	public static function displayMoneyFormat( $val, $numberFormat = true, $showInConfiguredDefaultCurrency = false, $displaySymbol = true,$stringFormat = false){
		...
		if( $showInConfiguredDefaultCurrency ){
			$currency_id = FatApp::getConfig('CONF_CURRENCY', FatUtility::VAR_INT, 1);
			$currencyData = Currency::getAttributesById($currency_id,array('currency_code','currency_symbol_left','currency_symbol_right','currency_value')
			);
	...
	static function isRenderTemplateExist($template = ''){
	...
	static function getWeightInGrams( $unit, $val ){
	...
	static function getLengthInCentimeter($val,$unit){
		$unit = FatUtility::int($unit);
		switch($unit){
	...
	static function getVolumeInCC( $unit, $val ){
		$unit = FatUtility::int($unit);
		return $val;
	}
	...
	static function currencyDisclaimer( $langId, $amount = 0 ){
	...
	static function getUserCookiesEnabled(){
