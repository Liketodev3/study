									...
									<?php foreach ($teachLanguages as $key => $teachLanguage) { ?>
										...
										<li class="<?php echo ($cartData['languageId'] == $key) ? 'is-active' : ''; ?>">
											<label class="selection">
												<span class="radio">
													<input onClick="addToCart('<?php echo $cartData['user_id'] ?>', '<?php echo $cartData['lpackage_id']; ?>','<?php echo $key; ?>', '', '', '<?php echo isset($cartData['grpcls_id']) ? $cartData['grpcls_id'] : 0 ?>',<?php echo FatApp::getConfig('CONF_DEFAULT_PAID_LESSON_DURATION', FatUtility::VAR_INT, 60); ?>);" type="radio" name="language" value="<?php echo $key; ?>" <?php echo ($cartData['languageId'] == $key) ? 'checked="checked"' : ''; ?>><i class="input-helper"></i>
												</span>
