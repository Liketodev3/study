	...
	<div class="table-scroll">
	 <table class="table myTeacherTable">
		...
		<?php $i=1; foreach($favouritesData['Favourites'] as $favourite){ 
			  $teacherDetailPageUrl = CommonHelper::generateUrl('Teachers', 'view', array($favourite['uft_teacher_id']) );		
		?>
<tr>
                                        <td width="35%">
														...
														<div class="avtar avtar--normal" data-text="<?php echo CommonHelper::getFirstChar($favourite['user_first_name']); ?>">
															<?php 
																if( true == User::isProfilePicUploaded( $favourite['uft_teacher_id'] ) ){
																	$img = CommonHelper::generateUrl('Image','user', array( $favourite['uft_teacher_id'] )).'?'.time(); 
																	echo '<img src="'.$img.'" />';
																}
															?>
														</div>
