...
class CheckoutController extends LoggedUserController{
	...
	public function confirmOrder(){
		...
		$orderData = array(
			...
			'order_net_amount'	=>	$cartData["orderNetAmount"],
			'order_is_wallet_selected'	=>	$cartData['cartWalletSelected'],
			'order_wallet_amount_charge' => $cartData['walletAmountCharge'],
			...
			'order_discount_total'		=>	$cartData['cartDiscounts']['coupon_discount_total'],
		...
		$userId = UserAuthentication::getLoggedUserId();
		$userWalletBalance = User::getUserBalance( $userId );
		if( $cartData['orderNetAmount'] > 0 && $cartData['cartWalletSelected'] && ( $userWalletBalance >= $cartData['orderNetAmount'] ) && !$pmethodId ){
