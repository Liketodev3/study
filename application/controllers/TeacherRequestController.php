...
class TeacherRequestController extends MyAppController {
	...
	public function index(){
		//$teacherRequestRow = TeacherRequest::getData( UserAuthentication::getLoggedUserId() );
	...
	public function form(){
		...
		if( false !== $teacherRequestRow && 
			$teacherRequestRow['utrequest_status'] == TeacherRequest::STATUS_APPROVED ){
		...
		if( false !== $teacherRequestRow &&
			( $teacherRequestRow['utrequest_status'] == TeacherRequest::STATUS_PENDING ) ){
	...
	public function setUpTeacherApproval(){
		...
		/* Profile Pic[ */
		if( !$fileHandlerObj->saveImage($_FILES['user_profile_pic']['tmp_name'], $fileHandlerObj::FILETYPE_TEACHER_APPROVAL_USER_PROFILE_IMAGE, 
		$userId, 0, $_FILES['user_profile_pic']['name'], -1, true )
		){
			if ( FatUtility::isAjaxCall() ) {
