...
class BlogPost extends MyAppModel{
	...
	public static function getSearchObject( $langId = 0, $joinCategory = true , $post_published = false , $categoryActive = false ) {
		...
		if( $langId > 0){
			...
			bp_l.'.static::DB_LANG_TBL_PREFIX.'lang_id = '.$langId,'bp_l');
		}
		
		if($joinCategory){
			$srch->joinTable(static::DB_POST_TO_CAT_TBL,'LEFT OUTER JOIN',
			'bptc.'.static::DB_POST_TO_CAT_TBL_PREFIX.'post_id = bp.'.static::tblFld('id'),'bptc');
			
			$srch->joinTable(BlogPostCategory::DB_TBL,'LEFT OUTER JOIN',
			'bptc.'.static::DB_POST_TO_CAT_TBL_PREFIX.'bpcategory_id = bpc.'.BlogPostCategory::tblFld('id').' and bpc.bpcategory_deleted =0','bpc');
			if( $langId > 0){
				$srch->joinTable(BlogPostCategory::DB_TBL_LANG,'LEFT OUTER JOIN',
			'bpc_l.'.BlogPostCategory::DB_LANG_TBL_PREFIX.'bpcategory_id = bpc.'.BlogPostCategory::tblFld('id').' and bpc_l.'.BlogPostCategory::DB_LANG_TBL_PREFIX.'lang_id = '.$langId,'bpc_l');
			}
