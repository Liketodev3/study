...
class Slides extends MyAppModel
{
    ...
    public static function getSlidesWithPromotionObj($langId = 0, $isActive = true)
    {
        ...
        $srch->joinTable(Promotion::DB_TBL, 'LEFT OUTER JOIN', 'sl.slide_type = ' . Slides::TYPE_PPC . ' and sl.slide_record_id = pr.promotion_id', 'pr');
        if ($langId) {
            $srch->joinTable(Promotion::DB_LANG_TBL, 'LEFT OUTER JOIN', 'pr_l.promotionlang_promotion_id = pr.promotion_id AND pr_l.promotionlang_lang_id = ' . $langId, 'pr_l');
        }
        $srch->addFld(['if(sl.slide_type = ' . Slides::TYPE_PPC . ',pr.promotion_start_date,"0000-00-00") as start_date']);
        $srch->addFld(['if(sl.slide_type = ' . Slides::TYPE_PPC . ',pr.promotion_end_date,"0000-00-00") as end_date']);
        $srch->addFld(['if(sl.slide_type = ' . Slides::TYPE_PPC . ',pr.promotion_start_time,"00:00:00") as start_time']);
        $srch->addFld(['if(sl.slide_type = ' . Slides::TYPE_PPC . ',pr.promotion_end_time,"00:00:00") as end_time']);
        $srch->addFld(['if(sl.slide_type = ' . Slides::TYPE_PPC . ',pr.promotion_duration,-1) as promotion_duration']);
        $srch->addFld(['if(sl.slide_type = ' . Slides::TYPE_PPC . ',pr.promotion_budget,-1) as promotion_budget']);
    ...
    public function addMinimiumWalletbalanceCondition($langId = 0)
    {
        ...
        $this->addFld(['IF(pr.promotion_id > 0, userBalance,' . FatApp::getConfig('CONF_PPC_MIN_WALLET_BALANCE') . ') AS userBalance']);
