...
class DiscountCoupons extends MyAppModel{
	...
	public static function getSearchObject($langId = 0 , $active = true ,$isDeleted = true) {
		...
		if ( $langId > 0) {
			...
			AND couponlang_lang_id = ' . $langId, 'dc_l');
		}
	...
	public static function getValidCoupons( $userId, $langId, $coupon_code = '',$orderId = '' ){
		...
		if( $userId <= 0 ){ trigger_error( Label::getLabel("ERR_User_id_is_mandatory",$langId), E_USER_ERROR ); }
		if( $langId <= 0 ){ trigger_error( Label::getLabel("ERR_Language_id_is_mandatory",$langId), E_USER_ERROR ); }
