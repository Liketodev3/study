<?php
class RescheduledLessonLog extends MyAppModel
{
    const DB_TBL = 'tbl_reschedule_lesson_log';
    const DB_TBL_PREFIX = 'reschleslog_';

    private  $scheduledLessonId;
    public function __construct(int $id = 0, int $scheduledLessonId = 0)
    {
        parent::__construct(static::DB_TBL, static::DB_TBL_PREFIX . 'id', $id);
        $this->scheduledLessonId =  FatUtility::int($scheduledLessonId);
    }

    public function save()
    {
        if ($this->getMainTableRecordId() == 0) {
            $this->setFldValue('reschleslog_added_on', date('Y-m-d H:i:s'));
        }

        return parent::save();
    }

}
